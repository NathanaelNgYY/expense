...
| H9 — CI | Complete to current platform limit | GitHub Actions runs lint, Deno typecheck of the ingest Edge Function, tests with enforced coverage thresholds, build, an initial-bundle size budget, a blocking high-severity dependency audit, and a gitleaks history scan — on pushes, pull requests, and a weekly schedule. Static regression coverage prevents the audit gate or the verify/RLS/E2E jobs from being silently removed. GitHub's API still rejects branch protection and rulesets for this private repository until the account upgrades to Pro or the repository becomes public. | `.github/workflows/ci.yml`, `src/ciWorkflow.test.ts`, `docs/testing/h9-ci-enforcement.tdd.md` |
...
1. Measure production Core Web Vitals before another M14 pass.
2. Select the next product/audit item from M1–M9 or M12–M16.

## Remaining audit items

- H9 repository-enforced required checks remain blocked until GitHub enables branch protection/rulesets for the private repository (currently an account-plan limitation).
