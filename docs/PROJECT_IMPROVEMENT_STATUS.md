...
| M17 — live RLS isolation tests | Partially complete | `entries`, `poker_sessions`, `ingest_tokens`, `ingest_status`, the five shared-budget tables, and the `join_budget` / `regenerate_invite_code` SECURITY DEFINER RPCs are tested against a real Postgres with every migration applied. A CI job parallel to `verify` is configured; its first remote run is pending the branch push. Browser E2E and accessibility checks remain. | `supabase/tests/rls/`, `.github/workflows/ci.yml` |
...
3. Add M17 browser E2E and accessibility checks. (Live RLS isolation tests are implemented; the first remote CI run is pending the branch push.)
...
- Live RLS: 48 isolation tests across 9 tables and 2 SECURITY DEFINER RPCs pass against a real local Postgres; the same suite is configured as a parallel CI job. These replaced `supabase/tests/ingest_visibility.test.ts`, which asserted that migration files *contained* policy substrings and would have stayed green if a policy were later dropped.
